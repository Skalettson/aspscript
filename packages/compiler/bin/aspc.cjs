...
const { analyzeBundle } = require('../bundle-analyzer.js')
...
function startDevServer() {
  ...
  const sourceDir = args[2] || './src'

  console.log(`🚀 Запуск AspScript dev сервера на порту ${port}`)
  console.log(`📁 Исходная директория: ${sourceDir}`)
  console.log(`🔥 Hot-reload включен`)

  try {
    // Здесь будет интеграция с dev сервером
    // Пока просто собираем проект
    buildProject()
    ...
    console.error('❌ Ошибка запуска dev сервера:', error.message)
    process.exit(1)
  }
}
...
Команды:
  ...
  dev [port] [dir]     Запустить dev сервер с hot-reload (порт 3000, директория ./src)
...
module.exports = { showHelp, buildProject, compileFile, initProject }
